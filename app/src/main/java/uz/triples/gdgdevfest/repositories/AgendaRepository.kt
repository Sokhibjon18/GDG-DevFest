...
class AgendaRepository(private val application: Application) {
    ...
    suspend fun updateAgenda() = withContext(Dispatchers.IO) {
        databaseFireBase.child("sessions").addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (sessionsItem in snapshot.children) {
                    ...
                    for (tags in sessionsItem.child("tag").children) {
                        tagsList += tags.value.toString()
                    }
